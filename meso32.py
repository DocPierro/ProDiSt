...
if __name__ == '__main__':
    ...
    sbib = SbIB("BPIC13_open_32", ev, pn)
    _,_ = sbib.compute_smcABC(N=50, initial_threshold=0.99, threshold_tolerance=0.0025, fixed_w={}, njob=32, v=0)

    ev = EventLog("rl_data/BPIC20_dd/BPIC20_dd.xes")
    pn = ev.discover_pn_inductive()
    sbib = SbIB("BPIC20_dd_32", ev, pn)
    _,_ = sbib.compute_smcABC(N=50, initial_threshold=0.99, threshold_tolerance=0.0025, fixed_w={}, njob=32, v=0)

    ev = EventLog("rl_data/roadfines/roadfines.xes")
    pn = ev.discover_pn_inductive()
    sbib = SbIB("roadfines_32", ev, pn)
    _,_ = sbib.compute_smcABC(N=50, initial_threshold=0.99, threshold_tolerance=0.0025, fixed_w={}, njob=32, v=0)